# Maintainer: Peter Jung <ptr1337@archlinux.org>
# Contributor: Denis Benato <benato.denis96@gmail.com>
# Contributor: William Edwards <shadowapex@gmail.com>

# This file was partially generated by holo-make-rust-pkgbuild.
# To update it, run holo-make-rust-pkgbuild on the crate sources
# and then manually merge the resulting PKGBUILD with this file.

pkgname=inputplumber
pkgver=0.76.1
pkgrel=1
pkgdesc="Open source input router and remapper daemon for Linux"
arch=('i686' 'pentium4' 'x86_64' 'arm' 'armv7h' 'armv6h' 'aarch64')
options=(!lto)
url="https://github.com/ShadowBlip/InputPlumber"
license=('GPL-3.0-only')
depends=(
  dbus
  gcc-libs
  glibc
  libevdev
  libiio
  systemd-libs
)
makedepends=(
  clang
  cmake
  git
  make
  rust
  cargo
  holo-rust-packaging-tools
)
source=('inputplumber-0.76.1.tar.gz::https://github.com/ShadowBlip/InputPlumber/archive/v0.76.1.tar.gz'
        'adler2-2.0.1.tar.gz::https://crates.io/api/v1/crates/adler2/2.0.1/download'
        'aho-corasick-1.1.4.tar.gz::https://crates.io/api/v1/crates/aho-corasick/1.1.4/download'
        'allocator-api2-0.2.21.tar.gz::https://crates.io/api/v1/crates/allocator-api2/0.2.21/download'
        'android_system_properties-0.1.5.tar.gz::https://crates.io/api/v1/crates/android_system_properties/0.1.5/download'
        'ansi-str-0.9.0.tar.gz::https://crates.io/api/v1/crates/ansi-str/0.9.0/download'
        'ansitok-0.3.0.tar.gz::https://crates.io/api/v1/crates/ansitok/0.3.0/download'
        'anstream-0.6.21.tar.gz::https://crates.io/api/v1/crates/anstream/0.6.21/download'
        'anstyle-1.0.13.tar.gz::https://crates.io/api/v1/crates/anstyle/1.0.13/download'
        'anstyle-parse-0.2.7.tar.gz::https://crates.io/api/v1/crates/anstyle-parse/0.2.7/download'
        'anstyle-query-1.1.5.tar.gz::https://crates.io/api/v1/crates/anstyle-query/1.1.5/download'
        'anstyle-wincon-3.0.11.tar.gz::https://crates.io/api/v1/crates/anstyle-wincon/3.0.11/download'
        'anyhow-1.0.100.tar.gz::https://crates.io/api/v1/crates/anyhow/1.0.100/download'
        'arrayvec-0.7.6.tar.gz::https://crates.io/api/v1/crates/arrayvec/0.7.6/download'
        'async-broadcast-0.7.2.tar.gz::https://crates.io/api/v1/crates/async-broadcast/0.7.2/download'
        'async-recursion-1.1.1.tar.gz::https://crates.io/api/v1/crates/async-recursion/1.1.1/download'
        'async-trait-0.1.89.tar.gz::https://crates.io/api/v1/crates/async-trait/0.1.89/download'
        'atomic-0.6.1.tar.gz::https://crates.io/api/v1/crates/atomic/0.6.1/download'
        'autocfg-1.5.0.tar.gz::https://crates.io/api/v1/crates/autocfg/1.5.0/download'
        'base64-0.22.1.tar.gz::https://crates.io/api/v1/crates/base64/0.22.1/download'
        'bindgen-0.71.1.tar.gz::https://crates.io/api/v1/crates/bindgen/0.71.1/download'
        'bit-set-0.5.3.tar.gz::https://crates.io/api/v1/crates/bit-set/0.5.3/download'
        'bit-vec-0.6.3.tar.gz::https://crates.io/api/v1/crates/bit-vec/0.6.3/download'
        'bitflags-1.3.2.tar.gz::https://crates.io/api/v1/crates/bitflags/1.3.2/download'
        'bitflags-2.10.0.tar.gz::https://crates.io/api/v1/crates/bitflags/2.10.0/download'
        'bitvec-1.0.1.tar.gz::https://crates.io/api/v1/crates/bitvec/1.0.1/download'
        'block-buffer-0.10.4.tar.gz::https://crates.io/api/v1/crates/block-buffer/0.10.4/download'
        'bumpalo-3.19.1.tar.gz::https://crates.io/api/v1/crates/bumpalo/3.19.1/download'
        'bytecount-0.6.9.tar.gz::https://crates.io/api/v1/crates/bytecount/0.6.9/download'
        'bytemuck-1.24.0.tar.gz::https://crates.io/api/v1/crates/bytemuck/1.24.0/download'
        'bytes-1.11.0.tar.gz::https://crates.io/api/v1/crates/bytes/1.11.0/download'
        'castaway-0.2.4.tar.gz::https://crates.io/api/v1/crates/castaway/0.2.4/download'
        'cc-1.2.51.tar.gz::https://crates.io/api/v1/crates/cc/1.2.51/download'
        'cexpr-0.6.0.tar.gz::https://crates.io/api/v1/crates/cexpr/0.6.0/download'
        'cfg-if-1.0.4.tar.gz::https://crates.io/api/v1/crates/cfg-if/1.0.4/download'
        'cfg_aliases-0.2.1.tar.gz::https://crates.io/api/v1/crates/cfg_aliases/0.2.1/download'
        'chrono-0.4.42.tar.gz::https://crates.io/api/v1/crates/chrono/0.4.42/download'
        'clang-sys-1.8.1.tar.gz::https://crates.io/api/v1/crates/clang-sys/1.8.1/download'
        'clap-4.5.54.tar.gz::https://crates.io/api/v1/crates/clap/4.5.54/download'
        'clap_builder-4.5.54.tar.gz::https://crates.io/api/v1/crates/clap_builder/4.5.54/download'
        'clap_complete-4.5.65.tar.gz::https://crates.io/api/v1/crates/clap_complete/4.5.65/download'
        'clap_derive-4.5.49.tar.gz::https://crates.io/api/v1/crates/clap_derive/4.5.49/download'
        'clap_lex-0.7.6.tar.gz::https://crates.io/api/v1/crates/clap_lex/0.7.6/download'
        'colorchoice-1.0.4.tar.gz::https://crates.io/api/v1/crates/colorchoice/1.0.4/download'
        'colored-3.0.0.tar.gz::https://crates.io/api/v1/crates/colored/3.0.0/download'
        'compact_str-0.9.0.tar.gz::https://crates.io/api/v1/crates/compact_str/0.9.0/download'
        'concurrent-queue-2.5.0.tar.gz::https://crates.io/api/v1/crates/concurrent-queue/2.5.0/download'
        'convert_case-0.10.0.tar.gz::https://crates.io/api/v1/crates/convert_case/0.10.0/download'
        'core-foundation-0.10.0.tar.gz::https://crates.io/api/v1/crates/core-foundation/0.10.0/download'
        'core-foundation-sys-0.8.7.tar.gz::https://crates.io/api/v1/crates/core-foundation-sys/0.8.7/download'
        'cpufeatures-0.2.17.tar.gz::https://crates.io/api/v1/crates/cpufeatures/0.2.17/download'
        'crc32fast-1.5.0.tar.gz::https://crates.io/api/v1/crates/crc32fast/1.5.0/download'
        'crossbeam-utils-0.8.21.tar.gz::https://crates.io/api/v1/crates/crossbeam-utils/0.8.21/download'
        'crossterm-0.29.0.tar.gz::https://crates.io/api/v1/crates/crossterm/0.29.0/download'
        'crossterm_winapi-0.9.1.tar.gz::https://crates.io/api/v1/crates/crossterm_winapi/0.9.1/download'
        'crypto-common-0.1.7.tar.gz::https://crates.io/api/v1/crates/crypto-common/0.1.7/download'
        'csscolorparser-0.6.2.tar.gz::https://crates.io/api/v1/crates/csscolorparser/0.6.2/download'
        'darling-0.20.11.tar.gz::https://crates.io/api/v1/crates/darling/0.20.11/download'
        'darling_core-0.20.11.tar.gz::https://crates.io/api/v1/crates/darling_core/0.20.11/download'
        'darling_macro-0.20.11.tar.gz::https://crates.io/api/v1/crates/darling_macro/0.20.11/download'
        'deltae-0.3.2.tar.gz::https://crates.io/api/v1/crates/deltae/0.3.2/download'
        'deranged-0.5.5.tar.gz::https://crates.io/api/v1/crates/deranged/0.5.5/download'
        'derive_more-2.1.1.tar.gz::https://crates.io/api/v1/crates/derive_more/2.1.1/download'
        'derive_more-impl-2.1.1.tar.gz::https://crates.io/api/v1/crates/derive_more-impl/2.1.1/download'
        'digest-0.10.7.tar.gz::https://crates.io/api/v1/crates/digest/0.10.7/download'
        'document-features-0.2.12.tar.gz::https://crates.io/api/v1/crates/document-features/0.2.12/download'
        'dyn-clone-1.0.20.tar.gz::https://crates.io/api/v1/crates/dyn-clone/1.0.20/download'
        'either-1.15.0.tar.gz::https://crates.io/api/v1/crates/either/1.15.0/download'
        'endi-1.1.1.tar.gz::https://crates.io/api/v1/crates/endi/1.1.1/download'
        'enumflags2-0.7.12.tar.gz::https://crates.io/api/v1/crates/enumflags2/0.7.12/download'
        'enumflags2_derive-0.7.12.tar.gz::https://crates.io/api/v1/crates/enumflags2_derive/0.7.12/download'
        'env_filter-0.1.4.tar.gz::https://crates.io/api/v1/crates/env_filter/0.1.4/download'
        'env_logger-0.11.8.tar.gz::https://crates.io/api/v1/crates/env_logger/0.11.8/download'
        'equivalent-1.0.2.tar.gz::https://crates.io/api/v1/crates/equivalent/1.0.2/download'
        'errno-0.3.14.tar.gz::https://crates.io/api/v1/crates/errno/0.3.14/download'
        'euclid-0.22.11.tar.gz::https://crates.io/api/v1/crates/euclid/0.22.11/download'
        'evdev-0.12.2.tar.gz::https://github.com/emberian/evdev/archive/42b58ee08508b7799322a13bf89121a1d29cf0a2.tar.gz'
        'event-listener-5.4.1.tar.gz::https://crates.io/api/v1/crates/event-listener/5.4.1/download'
        'event-listener-strategy-0.5.4.tar.gz::https://crates.io/api/v1/crates/event-listener-strategy/0.5.4/download'
        'fancy-regex-0.11.0.tar.gz::https://crates.io/api/v1/crates/fancy-regex/0.11.0/download'
        'fastrand-2.3.0.tar.gz::https://crates.io/api/v1/crates/fastrand/2.3.0/download'
        'filedescriptor-0.8.3.tar.gz::https://crates.io/api/v1/crates/filedescriptor/0.8.3/download'
        'find-msvc-tools-0.1.6.tar.gz::https://crates.io/api/v1/crates/find-msvc-tools/0.1.6/download'
        'finl_unicode-1.4.0.tar.gz::https://crates.io/api/v1/crates/finl_unicode/1.4.0/download'
        'fixedbitset-0.4.2.tar.gz::https://crates.io/api/v1/crates/fixedbitset/0.4.2/download'
        'flate2-1.1.5.tar.gz::https://crates.io/api/v1/crates/flate2/1.1.5/download'
        'fnv-1.0.7.tar.gz::https://crates.io/api/v1/crates/fnv/1.0.7/download'
        'foldhash-0.2.0.tar.gz::https://crates.io/api/v1/crates/foldhash/0.2.0/download'
        'funty-2.0.0.tar.gz::https://crates.io/api/v1/crates/funty/2.0.0/download'
        'futures-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures/0.3.31/download'
        'futures-channel-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-channel/0.3.31/download'
        'futures-core-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-core/0.3.31/download'
        'futures-executor-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-executor/0.3.31/download'
        'futures-io-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-io/0.3.31/download'
        'futures-lite-2.6.1.tar.gz::https://crates.io/api/v1/crates/futures-lite/2.6.1/download'
        'futures-macro-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-macro/0.3.31/download'
        'futures-sink-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-sink/0.3.31/download'
        'futures-task-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-task/0.3.31/download'
        'futures-util-0.3.31.tar.gz::https://crates.io/api/v1/crates/futures-util/0.3.31/download'
        'generic-array-0.14.7.tar.gz::https://crates.io/api/v1/crates/generic-array/0.14.7/download'
        'getrandom-0.3.4.tar.gz::https://crates.io/api/v1/crates/getrandom/0.3.4/download'
        'glob-0.3.3.tar.gz::https://crates.io/api/v1/crates/glob/0.3.3/download'
        'glob-match-0.2.1.tar.gz::https://crates.io/api/v1/crates/glob-match/0.2.1/download'
        'hashbrown-0.16.1.tar.gz::https://crates.io/api/v1/crates/hashbrown/0.16.1/download'
        'heck-0.5.0.tar.gz::https://crates.io/api/v1/crates/heck/0.5.0/download'
        'hermit-abi-0.3.9.tar.gz::https://crates.io/api/v1/crates/hermit-abi/0.3.9/download'
        'hex-0.4.3.tar.gz::https://crates.io/api/v1/crates/hex/0.4.3/download'
        'hidapi-2.6.4.tar.gz::https://crates.io/api/v1/crates/hidapi/2.6.4/download'
        'iana-time-zone-0.1.64.tar.gz::https://crates.io/api/v1/crates/iana-time-zone/0.1.64/download'
        'iana-time-zone-haiku-0.1.2.tar.gz::https://crates.io/api/v1/crates/iana-time-zone-haiku/0.1.2/download'
        'ident_case-1.0.1.tar.gz::https://crates.io/api/v1/crates/ident_case/1.0.1/download'
        'indexmap-2.12.1.tar.gz::https://crates.io/api/v1/crates/indexmap/2.12.1/download'
        'indoc-2.0.7.tar.gz::https://crates.io/api/v1/crates/indoc/2.0.7/download'
        'industrial-io-0.6.1.tar.gz::https://crates.io/api/v1/crates/industrial-io/0.6.1/download'
        'inotify-0.11.0.tar.gz::https://crates.io/api/v1/crates/inotify/0.11.0/download'
        'inotify-sys-0.1.5.tar.gz::https://crates.io/api/v1/crates/inotify-sys/0.1.5/download'
        'instability-0.3.10.tar.gz::https://crates.io/api/v1/crates/instability/0.3.10/download'
        'io-extras-0.18.4.tar.gz::https://crates.io/api/v1/crates/io-extras/0.18.4/download'
        'io-kit-sys-0.4.1.tar.gz::https://crates.io/api/v1/crates/io-kit-sys/0.4.1/download'
        'io-lifetimes-1.0.11.tar.gz::https://crates.io/api/v1/crates/io-lifetimes/1.0.11/download'
        'io-lifetimes-2.0.4.tar.gz::https://crates.io/api/v1/crates/io-lifetimes/2.0.4/download'
        'is_terminal_polyfill-1.70.2.tar.gz::https://crates.io/api/v1/crates/is_terminal_polyfill/1.70.2/download'
        'itertools-0.13.0.tar.gz::https://crates.io/api/v1/crates/itertools/0.13.0/download'
        'itertools-0.14.0.tar.gz::https://crates.io/api/v1/crates/itertools/0.14.0/download'
        'itoa-1.0.17.tar.gz::https://crates.io/api/v1/crates/itoa/1.0.17/download'
        'jiff-0.2.17.tar.gz::https://crates.io/api/v1/crates/jiff/0.2.17/download'
        'jiff-static-0.2.17.tar.gz::https://crates.io/api/v1/crates/jiff-static/0.2.17/download'
        'js-sys-0.3.83.tar.gz::https://crates.io/api/v1/crates/js-sys/0.3.83/download'
        'kasuari-0.4.11.tar.gz::https://crates.io/api/v1/crates/kasuari/0.4.11/download'
        'lab-0.11.0.tar.gz::https://crates.io/api/v1/crates/lab/0.11.0/download'
        'lazy_static-1.5.0.tar.gz::https://crates.io/api/v1/crates/lazy_static/1.5.0/download'
        'libc-0.2.178.tar.gz::https://crates.io/api/v1/crates/libc/0.2.178/download'
        'libiio-sys-0.4.1.tar.gz::https://crates.io/api/v1/crates/libiio-sys/0.4.1/download'
        'libudev-0.3.0.tar.gz::https://crates.io/api/v1/crates/libudev/0.3.0/download'
        'libudev-sys-0.1.4.tar.gz::https://crates.io/api/v1/crates/libudev-sys/0.1.4/download'
        'line-clipping-0.3.5.tar.gz::https://crates.io/api/v1/crates/line-clipping/0.3.5/download'
        'linux-raw-sys-0.11.0.tar.gz::https://crates.io/api/v1/crates/linux-raw-sys/0.11.0/download'
        'litrs-1.0.0.tar.gz::https://crates.io/api/v1/crates/litrs/1.0.0/download'
        'lock_api-0.4.14.tar.gz::https://crates.io/api/v1/crates/lock_api/0.4.14/download'
        'log-0.4.29.tar.gz::https://crates.io/api/v1/crates/log/0.4.29/download'
        'lru-0.16.2.tar.gz::https://crates.io/api/v1/crates/lru/0.16.2/download'
        'mac_address-1.1.8.tar.gz::https://crates.io/api/v1/crates/mac_address/1.1.8/download'
        'mach2-0.4.3.tar.gz::https://crates.io/api/v1/crates/mach2/0.4.3/download'
        'memchr-2.7.6.tar.gz::https://crates.io/api/v1/crates/memchr/2.7.6/download'
        'memmem-0.1.1.tar.gz::https://crates.io/api/v1/crates/memmem/0.1.1/download'
        'memoffset-0.7.1.tar.gz::https://crates.io/api/v1/crates/memoffset/0.7.1/download'
        'memoffset-0.9.1.tar.gz::https://crates.io/api/v1/crates/memoffset/0.9.1/download'
        'minimal-lexical-0.2.1.tar.gz::https://crates.io/api/v1/crates/minimal-lexical/0.2.1/download'
        'miniz_oxide-0.8.9.tar.gz::https://crates.io/api/v1/crates/miniz_oxide/0.8.9/download'
        'mio-1.1.1.tar.gz::https://crates.io/api/v1/crates/mio/1.1.1/download'
        'nix-0.26.4.tar.gz::https://crates.io/api/v1/crates/nix/0.26.4/download'
        'nix-0.29.0.tar.gz::https://crates.io/api/v1/crates/nix/0.29.0/download'
        'nix-0.30.1.tar.gz::https://crates.io/api/v1/crates/nix/0.30.1/download'
        'nom-7.1.3.tar.gz::https://crates.io/api/v1/crates/nom/7.1.3/download'
        'num-conv-0.1.0.tar.gz::https://crates.io/api/v1/crates/num-conv/0.1.0/download'
        'num-derive-0.4.2.tar.gz::https://crates.io/api/v1/crates/num-derive/0.4.2/download'
        'num-traits-0.2.19.tar.gz::https://crates.io/api/v1/crates/num-traits/0.2.19/download'
        'num_threads-0.1.7.tar.gz::https://crates.io/api/v1/crates/num_threads/0.1.7/download'
        'once_cell-1.21.3.tar.gz::https://crates.io/api/v1/crates/once_cell/1.21.3/download'
        'once_cell_polyfill-1.70.2.tar.gz::https://crates.io/api/v1/crates/once_cell_polyfill/1.70.2/download'
        'ordered-float-4.6.0.tar.gz::https://crates.io/api/v1/crates/ordered-float/4.6.0/download'
        'ordered-stream-0.2.0.tar.gz::https://crates.io/api/v1/crates/ordered-stream/0.2.0/download'
        'packed_struct-0.10.1.tar.gz::https://crates.io/api/v1/crates/packed_struct/0.10.1/download'
        'packed_struct_codegen-0.10.1.tar.gz::https://crates.io/api/v1/crates/packed_struct_codegen/0.10.1/download'
        'papergrid-0.17.0.tar.gz::https://crates.io/api/v1/crates/papergrid/0.17.0/download'
        'parking-2.2.1.tar.gz::https://crates.io/api/v1/crates/parking/2.2.1/download'
        'parking_lot-0.12.5.tar.gz::https://crates.io/api/v1/crates/parking_lot/0.12.5/download'
        'parking_lot_core-0.9.12.tar.gz::https://crates.io/api/v1/crates/parking_lot_core/0.9.12/download'
        'pest-2.8.4.tar.gz::https://crates.io/api/v1/crates/pest/2.8.4/download'
        'pest_derive-2.8.4.tar.gz::https://crates.io/api/v1/crates/pest_derive/2.8.4/download'
        'pest_generator-2.8.4.tar.gz::https://crates.io/api/v1/crates/pest_generator/2.8.4/download'
        'pest_meta-2.8.4.tar.gz::https://crates.io/api/v1/crates/pest_meta/2.8.4/download'
        'phf-0.11.3.tar.gz::https://crates.io/api/v1/crates/phf/0.11.3/download'
        'phf_codegen-0.11.3.tar.gz::https://crates.io/api/v1/crates/phf_codegen/0.11.3/download'
        'phf_generator-0.11.3.tar.gz::https://crates.io/api/v1/crates/phf_generator/0.11.3/download'
        'phf_macros-0.11.3.tar.gz::https://crates.io/api/v1/crates/phf_macros/0.11.3/download'
        'phf_shared-0.11.3.tar.gz::https://crates.io/api/v1/crates/phf_shared/0.11.3/download'
        'pin-project-lite-0.2.16.tar.gz::https://crates.io/api/v1/crates/pin-project-lite/0.2.16/download'
        'pin-utils-0.1.0.tar.gz::https://crates.io/api/v1/crates/pin-utils/0.1.0/download'
        'pkg-config-0.3.32.tar.gz::https://crates.io/api/v1/crates/pkg-config/0.3.32/download'
        'portable-atomic-1.13.0.tar.gz::https://crates.io/api/v1/crates/portable-atomic/1.13.0/download'
        'portable-atomic-util-0.2.4.tar.gz::https://crates.io/api/v1/crates/portable-atomic-util/0.2.4/download'
        'powerfmt-0.2.0.tar.gz::https://crates.io/api/v1/crates/powerfmt/0.2.0/download'
        'ppv-lite86-0.2.21.tar.gz::https://crates.io/api/v1/crates/ppv-lite86/0.2.21/download'
        'proc-macro-crate-3.4.0.tar.gz::https://crates.io/api/v1/crates/proc-macro-crate/3.4.0/download'
        'proc-macro-error-attr2-2.0.0.tar.gz::https://crates.io/api/v1/crates/proc-macro-error-attr2/2.0.0/download'
        'proc-macro-error2-2.0.1.tar.gz::https://crates.io/api/v1/crates/proc-macro-error2/2.0.1/download'
        'proc-macro2-1.0.104.tar.gz::https://crates.io/api/v1/crates/proc-macro2/1.0.104/download'
        'procfs-0.18.0.tar.gz::https://crates.io/api/v1/crates/procfs/0.18.0/download'
        'procfs-core-0.18.0.tar.gz::https://crates.io/api/v1/crates/procfs-core/0.18.0/download'
        'quote-1.0.40.tar.gz::https://crates.io/api/v1/crates/quote/1.0.40/download'
        'r-efi-5.3.0.tar.gz::https://crates.io/api/v1/crates/r-efi/5.3.0/download'
        'radium-0.7.0.tar.gz::https://crates.io/api/v1/crates/radium/0.7.0/download'
        'rand-0.8.5.tar.gz::https://crates.io/api/v1/crates/rand/0.8.5/download'
        'rand-0.9.2.tar.gz::https://crates.io/api/v1/crates/rand/0.9.2/download'
        'rand_chacha-0.9.0.tar.gz::https://crates.io/api/v1/crates/rand_chacha/0.9.0/download'
        'rand_core-0.6.4.tar.gz::https://crates.io/api/v1/crates/rand_core/0.6.4/download'
        'rand_core-0.9.3.tar.gz::https://crates.io/api/v1/crates/rand_core/0.9.3/download'
        'ratatui-0.30.0.tar.gz::https://crates.io/api/v1/crates/ratatui/0.30.0/download'
        'ratatui-core-0.1.0.tar.gz::https://crates.io/api/v1/crates/ratatui-core/0.1.0/download'
        'ratatui-crossterm-0.1.0.tar.gz::https://crates.io/api/v1/crates/ratatui-crossterm/0.1.0/download'
        'ratatui-macros-0.7.0.tar.gz::https://crates.io/api/v1/crates/ratatui-macros/0.7.0/download'
        'ratatui-termwiz-0.1.0.tar.gz::https://crates.io/api/v1/crates/ratatui-termwiz/0.1.0/download'
        'ratatui-widgets-0.3.0.tar.gz::https://crates.io/api/v1/crates/ratatui-widgets/0.3.0/download'
        'redox_syscall-0.5.18.tar.gz::https://crates.io/api/v1/crates/redox_syscall/0.5.18/download'
        'ref-cast-1.0.25.tar.gz::https://crates.io/api/v1/crates/ref-cast/1.0.25/download'
        'ref-cast-impl-1.0.25.tar.gz::https://crates.io/api/v1/crates/ref-cast-impl/1.0.25/download'
        'regex-1.12.2.tar.gz::https://crates.io/api/v1/crates/regex/1.12.2/download'
        'regex-automata-0.4.13.tar.gz::https://crates.io/api/v1/crates/regex-automata/0.4.13/download'
        'regex-syntax-0.8.8.tar.gz::https://crates.io/api/v1/crates/regex-syntax/0.8.8/download'
        'rustc-hash-2.1.1.tar.gz::https://crates.io/api/v1/crates/rustc-hash/2.1.1/download'
        'rustc_version-0.4.1.tar.gz::https://crates.io/api/v1/crates/rustc_version/0.4.1/download'
        'rustix-1.1.3.tar.gz::https://crates.io/api/v1/crates/rustix/1.1.3/download'
        'rustversion-1.0.22.tar.gz::https://crates.io/api/v1/crates/rustversion/1.0.22/download'
        'ryu-1.0.22.tar.gz::https://crates.io/api/v1/crates/ryu/1.0.22/download'
        'schemars-1.2.0.tar.gz::https://crates.io/api/v1/crates/schemars/1.2.0/download'
        'schemars_derive-1.2.0.tar.gz::https://crates.io/api/v1/crates/schemars_derive/1.2.0/download'
        'scopeguard-1.2.0.tar.gz::https://crates.io/api/v1/crates/scopeguard/1.2.0/download'
        'semver-1.0.27.tar.gz::https://crates.io/api/v1/crates/semver/1.0.27/download'
        'serde-1.0.228.tar.gz::https://crates.io/api/v1/crates/serde/1.0.228/download'
        'serde_core-1.0.228.tar.gz::https://crates.io/api/v1/crates/serde_core/1.0.228/download'
        'serde_derive-1.0.228.tar.gz::https://crates.io/api/v1/crates/serde_derive/1.0.228/download'
        'serde_derive_internals-0.29.1.tar.gz::https://crates.io/api/v1/crates/serde_derive_internals/0.29.1/download'
        'serde_json-1.0.149.tar.gz::https://crates.io/api/v1/crates/serde_json/1.0.149/download'
        'serde_repr-0.1.20.tar.gz::https://crates.io/api/v1/crates/serde_repr/0.1.20/download'
        'serde_yaml-0.9.34+deprecated.tar.gz::https://crates.io/api/v1/crates/serde_yaml/0.9.34+deprecated/download'
        'serialport-4.8.1.tar.gz::https://crates.io/api/v1/crates/serialport/4.8.1/download'
        'sha2-0.10.9.tar.gz::https://crates.io/api/v1/crates/sha2/0.10.9/download'
        'shlex-1.3.0.tar.gz::https://crates.io/api/v1/crates/shlex/1.3.0/download'
        'signal-hook-0.3.18.tar.gz::https://crates.io/api/v1/crates/signal-hook/0.3.18/download'
        'signal-hook-mio-0.2.5.tar.gz::https://crates.io/api/v1/crates/signal-hook-mio/0.2.5/download'
        'signal-hook-registry-1.4.8.tar.gz::https://crates.io/api/v1/crates/signal-hook-registry/1.4.8/download'
        'simd-adler32-0.3.8.tar.gz::https://crates.io/api/v1/crates/simd-adler32/0.3.8/download'
        'simple_logger-5.1.0.tar.gz::https://crates.io/api/v1/crates/simple_logger/5.1.0/download'
        'siphasher-1.0.1.tar.gz::https://crates.io/api/v1/crates/siphasher/1.0.1/download'
        'slab-0.4.11.tar.gz::https://crates.io/api/v1/crates/slab/0.4.11/download'
        'smallvec-1.15.1.tar.gz::https://crates.io/api/v1/crates/smallvec/1.15.1/download'
        'socket2-0.6.1.tar.gz::https://crates.io/api/v1/crates/socket2/0.6.1/download'
        'socketpair-0.19.8.tar.gz::https://crates.io/api/v1/crates/socketpair/0.19.8/download'
        'static_assertions-1.1.0.tar.gz::https://crates.io/api/v1/crates/static_assertions/1.1.0/download'
        'strsim-0.11.1.tar.gz::https://crates.io/api/v1/crates/strsim/0.11.1/download'
        'strum-0.27.2.tar.gz::https://crates.io/api/v1/crates/strum/0.27.2/download'
        'strum_macros-0.27.2.tar.gz::https://crates.io/api/v1/crates/strum_macros/0.27.2/download'
        'syn-1.0.109.tar.gz::https://crates.io/api/v1/crates/syn/1.0.109/download'
        'syn-2.0.111.tar.gz::https://crates.io/api/v1/crates/syn/2.0.111/download'
        'tabled-0.20.0.tar.gz::https://crates.io/api/v1/crates/tabled/0.20.0/download'
        'tabled_derive-0.11.0.tar.gz::https://crates.io/api/v1/crates/tabled_derive/0.11.0/download'
        'tap-1.0.1.tar.gz::https://crates.io/api/v1/crates/tap/1.0.1/download'
        'tempfile-3.24.0.tar.gz::https://crates.io/api/v1/crates/tempfile/3.24.0/download'
        'terminfo-0.9.0.tar.gz::https://crates.io/api/v1/crates/terminfo/0.9.0/download'
        'termios-0.3.3.tar.gz::https://crates.io/api/v1/crates/termios/0.3.3/download'
        'termwiz-0.23.3.tar.gz::https://crates.io/api/v1/crates/termwiz/0.23.3/download'
        'testing_table-0.3.0.tar.gz::https://crates.io/api/v1/crates/testing_table/0.3.0/download'
        'thiserror-1.0.69.tar.gz::https://crates.io/api/v1/crates/thiserror/1.0.69/download'
        'thiserror-2.0.17.tar.gz::https://crates.io/api/v1/crates/thiserror/2.0.17/download'
        'thiserror-impl-1.0.69.tar.gz::https://crates.io/api/v1/crates/thiserror-impl/1.0.69/download'
        'thiserror-impl-2.0.17.tar.gz::https://crates.io/api/v1/crates/thiserror-impl/2.0.17/download'
        'time-0.3.44.tar.gz::https://crates.io/api/v1/crates/time/0.3.44/download'
        'time-core-0.1.6.tar.gz::https://crates.io/api/v1/crates/time-core/0.1.6/download'
        'time-macros-0.2.24.tar.gz::https://crates.io/api/v1/crates/time-macros/0.2.24/download'
        'tokio-1.49.0.tar.gz::https://crates.io/api/v1/crates/tokio/1.49.0/download'
        'tokio-macros-2.6.0.tar.gz::https://crates.io/api/v1/crates/tokio-macros/2.6.0/download'
        'toml_datetime-0.7.5+spec-1.1.0.tar.gz::https://crates.io/api/v1/crates/toml_datetime/0.7.5+spec-1.1.0/download'
        'toml_edit-0.23.10+spec-1.0.0.tar.gz::https://crates.io/api/v1/crates/toml_edit/0.23.10+spec-1.0.0/download'
        'toml_parser-1.0.6+spec-1.1.0.tar.gz::https://crates.io/api/v1/crates/toml_parser/1.0.6+spec-1.1.0/download'
        'tracing-0.1.44.tar.gz::https://crates.io/api/v1/crates/tracing/0.1.44/download'
        'tracing-attributes-0.1.31.tar.gz::https://crates.io/api/v1/crates/tracing-attributes/0.1.31/download'
        'tracing-core-0.1.36.tar.gz::https://crates.io/api/v1/crates/tracing-core/0.1.36/download'
        'typenum-1.19.0.tar.gz::https://crates.io/api/v1/crates/typenum/1.19.0/download'
        'ucd-trie-0.1.7.tar.gz::https://crates.io/api/v1/crates/ucd-trie/0.1.7/download'
        'udev-0.9.3.tar.gz::https://crates.io/api/v1/crates/udev/0.9.3/download'
        'uds_windows-1.1.0.tar.gz::https://crates.io/api/v1/crates/uds_windows/1.1.0/download'
        'uhid-virt-0.0.8.tar.gz::https://crates.io/api/v1/crates/uhid-virt/0.0.8/download'
        'uhidrs-sys-1.0.4.tar.gz::https://crates.io/api/v1/crates/uhidrs-sys/1.0.4/download'
        'unescaper-0.1.8.tar.gz::https://crates.io/api/v1/crates/unescaper/0.1.8/download'
        'unicode-ident-1.0.22.tar.gz::https://crates.io/api/v1/crates/unicode-ident/1.0.22/download'
        'unicode-segmentation-1.12.0.tar.gz::https://crates.io/api/v1/crates/unicode-segmentation/1.12.0/download'
        'unicode-truncate-2.0.0.tar.gz::https://crates.io/api/v1/crates/unicode-truncate/2.0.0/download'
        'unicode-width-0.2.2.tar.gz::https://crates.io/api/v1/crates/unicode-width/0.2.2/download'
        'unsafe-libyaml-0.2.11.tar.gz::https://crates.io/api/v1/crates/unsafe-libyaml/0.2.11/download'
        'utf8parse-0.2.2.tar.gz::https://crates.io/api/v1/crates/utf8parse/0.2.2/download'
        'uuid-1.19.0.tar.gz::https://crates.io/api/v1/crates/uuid/1.19.0/download'
        'version-compare-0.2.1.tar.gz::https://crates.io/api/v1/crates/version-compare/0.2.1/download'
        'version_check-0.9.5.tar.gz::https://crates.io/api/v1/crates/version_check/0.9.5/download'
        'virtual-usb-0.1.0.tar.gz::https://github.com/ShadowBlip/virtual-usb-rs/archive/5c4c551a23b56f627a36d6775a5876c174be9eb3.tar.gz'
        'vte-0.14.1.tar.gz::https://crates.io/api/v1/crates/vte/0.14.1/download'
        'vtparse-0.6.2.tar.gz::https://crates.io/api/v1/crates/vtparse/0.6.2/download'
        'wasi-0.11.1+wasi-snapshot-preview1.tar.gz::https://crates.io/api/v1/crates/wasi/0.11.1+wasi-snapshot-preview1/download'
        'wasip2-1.0.1+wasi-0.2.4.tar.gz::https://crates.io/api/v1/crates/wasip2/1.0.1+wasi-0.2.4/download'
        'wasm-bindgen-0.2.106.tar.gz::https://crates.io/api/v1/crates/wasm-bindgen/0.2.106/download'
        'wasm-bindgen-macro-0.2.106.tar.gz::https://crates.io/api/v1/crates/wasm-bindgen-macro/0.2.106/download'
        'wasm-bindgen-macro-support-0.2.106.tar.gz::https://crates.io/api/v1/crates/wasm-bindgen-macro-support/0.2.106/download'
        'wasm-bindgen-shared-0.2.106.tar.gz::https://crates.io/api/v1/crates/wasm-bindgen-shared/0.2.106/download'
        'wezterm-bidi-0.2.3.tar.gz::https://crates.io/api/v1/crates/wezterm-bidi/0.2.3/download'
        'wezterm-blob-leases-0.1.1.tar.gz::https://crates.io/api/v1/crates/wezterm-blob-leases/0.1.1/download'
        'wezterm-color-types-0.3.0.tar.gz::https://crates.io/api/v1/crates/wezterm-color-types/0.3.0/download'
        'wezterm-dynamic-0.2.1.tar.gz::https://crates.io/api/v1/crates/wezterm-dynamic/0.2.1/download'
        'wezterm-dynamic-derive-0.1.1.tar.gz::https://crates.io/api/v1/crates/wezterm-dynamic-derive/0.1.1/download'
        'wezterm-input-types-0.1.0.tar.gz::https://crates.io/api/v1/crates/wezterm-input-types/0.1.0/download'
        'winapi-0.3.9.tar.gz::https://crates.io/api/v1/crates/winapi/0.3.9/download'
        'winapi-i686-pc-windows-gnu-0.4.0.tar.gz::https://crates.io/api/v1/crates/winapi-i686-pc-windows-gnu/0.4.0/download'
        'winapi-x86_64-pc-windows-gnu-0.4.0.tar.gz::https://crates.io/api/v1/crates/winapi-x86_64-pc-windows-gnu/0.4.0/download'
        'windows-core-0.62.2.tar.gz::https://crates.io/api/v1/crates/windows-core/0.62.2/download'
        'windows-implement-0.60.2.tar.gz::https://crates.io/api/v1/crates/windows-implement/0.60.2/download'
        'windows-interface-0.59.3.tar.gz::https://crates.io/api/v1/crates/windows-interface/0.59.3/download'
        'windows-link-0.2.1.tar.gz::https://crates.io/api/v1/crates/windows-link/0.2.1/download'
        'windows-result-0.4.1.tar.gz::https://crates.io/api/v1/crates/windows-result/0.4.1/download'
        'windows-strings-0.5.1.tar.gz::https://crates.io/api/v1/crates/windows-strings/0.5.1/download'
        'windows-sys-0.48.0.tar.gz::https://crates.io/api/v1/crates/windows-sys/0.48.0/download'
        'windows-sys-0.52.0.tar.gz::https://crates.io/api/v1/crates/windows-sys/0.52.0/download'
        'windows-sys-0.59.0.tar.gz::https://crates.io/api/v1/crates/windows-sys/0.59.0/download'
        'windows-sys-0.60.2.tar.gz::https://crates.io/api/v1/crates/windows-sys/0.60.2/download'
        'windows-sys-0.61.2.tar.gz::https://crates.io/api/v1/crates/windows-sys/0.61.2/download'
        'windows-targets-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows-targets/0.48.5/download'
        'windows-targets-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows-targets/0.52.6/download'
        'windows-targets-0.53.5.tar.gz::https://crates.io/api/v1/crates/windows-targets/0.53.5/download'
        'windows_aarch64_gnullvm-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_aarch64_gnullvm/0.48.5/download'
        'windows_aarch64_gnullvm-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_aarch64_gnullvm/0.52.6/download'
        'windows_aarch64_gnullvm-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_aarch64_gnullvm/0.53.1/download'
        'windows_aarch64_msvc-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_aarch64_msvc/0.48.5/download'
        'windows_aarch64_msvc-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_aarch64_msvc/0.52.6/download'
        'windows_aarch64_msvc-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_aarch64_msvc/0.53.1/download'
        'windows_i686_gnu-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_i686_gnu/0.48.5/download'
        'windows_i686_gnu-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_i686_gnu/0.52.6/download'
        'windows_i686_gnu-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_i686_gnu/0.53.1/download'
        'windows_i686_gnullvm-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_i686_gnullvm/0.52.6/download'
        'windows_i686_gnullvm-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_i686_gnullvm/0.53.1/download'
        'windows_i686_msvc-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_i686_msvc/0.48.5/download'
        'windows_i686_msvc-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_i686_msvc/0.52.6/download'
        'windows_i686_msvc-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_i686_msvc/0.53.1/download'
        'windows_x86_64_gnu-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_gnu/0.48.5/download'
        'windows_x86_64_gnu-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_gnu/0.52.6/download'
        'windows_x86_64_gnu-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_gnu/0.53.1/download'
        'windows_x86_64_gnullvm-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_gnullvm/0.48.5/download'
        'windows_x86_64_gnullvm-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_gnullvm/0.52.6/download'
        'windows_x86_64_gnullvm-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_gnullvm/0.53.1/download'
        'windows_x86_64_msvc-0.48.5.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_msvc/0.48.5/download'
        'windows_x86_64_msvc-0.52.6.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_msvc/0.52.6/download'
        'windows_x86_64_msvc-0.53.1.tar.gz::https://crates.io/api/v1/crates/windows_x86_64_msvc/0.53.1/download'
        'winnow-0.7.14.tar.gz::https://crates.io/api/v1/crates/winnow/0.7.14/download'
        'wit-bindgen-0.46.0.tar.gz::https://crates.io/api/v1/crates/wit-bindgen/0.46.0/download'
        'wyz-0.5.1.tar.gz::https://crates.io/api/v1/crates/wyz/0.5.1/download'
        'xdg-3.0.0.tar.gz::https://crates.io/api/v1/crates/xdg/3.0.0/download'
        'zbus-5.12.0.tar.gz::https://crates.io/api/v1/crates/zbus/5.12.0/download'
        'zbus_macros-5.12.0.tar.gz::https://crates.io/api/v1/crates/zbus_macros/5.12.0/download'
        'zbus_names-4.2.0.tar.gz::https://crates.io/api/v1/crates/zbus_names/4.2.0/download'
        'zerocopy-0.8.31.tar.gz::https://crates.io/api/v1/crates/zerocopy/0.8.31/download'
        'zerocopy-derive-0.8.31.tar.gz::https://crates.io/api/v1/crates/zerocopy-derive/0.8.31/download'
        'zmij-1.0.3.tar.gz::https://crates.io/api/v1/crates/zmij/1.0.3/download'
        'zvariant-5.8.0.tar.gz::https://crates.io/api/v1/crates/zvariant/5.8.0/download'
        'zvariant_derive-5.8.0.tar.gz::https://crates.io/api/v1/crates/zvariant_derive/5.8.0/download'
        'zvariant_utils-3.2.1.tar.gz::https://crates.io/api/v1/crates/zvariant_utils/3.2.1/download')

sha256sums=('e28b631cfa28ab37314bdc80e89a3b1aed546bd0e18c2f70e0099dba20cce179'
            '320119579fcad9c21884f5c4861d16174d0e06250625266f50fe6898340abefa'
            'ddd31a130427c27518df266943a5308ed92d4b226cc639f5a8f1002816174301'
            '683d7910e743518b0e34f1186f92494becacb047c7b6bf616c96772180fef923'
            '819e7219dbd41043ac279b19830f2efc897156490d7fd6ea916720117ee66311'
            '060de1453b69f46304b28274f382132f4e72c55637cf362920926a70d090890d'
            'c0a8acea8c2f1c60f0a92a8cd26bf96ca97db56f10bbcab238bbe0cceba659ee'
            '43d5b281e737544384e969a5ccad3f1cdd24b48086a0fc1b2a5262a26b8f4f4a'
            '5192cca8006f1fd4f7237516f40fa183bb07f8fbdfedaa0036de5ea9b0b45e78'
            '4e7644824f0aa2c7b9384579234ef10eb7efb6a0deb83f9630a49594dd9c15c2'
            '40c48f72fd53cd289104fc64099abca73db4166ad86ea0b4341abe65af83dadc'
            '291e6a250ff86cd4a820112fb8898808a366d8f9f58ce16d1f538353ad55747d'
            'a23eb6b1614318a8071c9b2521f36b424b2c83db5eb3a0fead4a6c0809af6e61'
            '7c02d123df017efcdfbd739ef81735b36c5ba83ec3c59c80a9d7ecc718f92e50'
            '435a87a52755b8f27fcf321ac4f04b2802e337c8c4872923137471ec39c37532'
            '3b43422f69d8ff38f95f1b2bb76517c91589a924d1559a0e935d7c8ce0274c11'
            '9035ad2d096bed7955a320ee7e2230574d28fd3c3a0f186cbea1ff3c7eed5dbb'
            'a89cbf775b137e9b968e67227ef7f775587cde3fd31b0d8599dbd0f598a48340'
            'c08606f8c3cbf4ce6ec8e28fb0014a2c086708fe954eaa885384a6165172e7e8'
            '72b3254f16251a8381aa12e40e3c4d2f0199f8c6508fbecb9d91f575e0fbb8c6'
            '5f58bf3d7db68cfbac37cfc485a8d711e87e064c3d0fe0435b92f7a407f9d6b3'
            '0700ddab506f33b20a03b13996eccd309a48e5ff77d0d95926aa0210fb4e95f1'
            '349f9b6a179ed607305526ca489b34ad0a41aed5f7980fa90eb03160b69598fb'
            'bef38d45163c2f1dde094a7dfd33ccf595c92905c8f8f4fdc18d06fb1037718a'
            '812e12b5285cc515a9c72a5c1d3b6d46a19dac5acfef5265968c166106e31dd3'
            '1bc2832c24239b0141d5674bb9174f9d68a8b5b3f2753311927c172ca46f7e9c'
            '3078c7629b62d3f0439517fa394996acacc5cbc91c5a20d8c658e77abd503a71'
            '5dd9dc738b7a8311c7ade152424974d8115f2cdad61e8dab8dac9f2362298510'
            '175812e0be2bccb6abe50bb8d566126198344f707e304f45c648fd8f2cc0365e'
            '1fbdf580320f38b612e485521afda1ee26d10cc9884efaaa750d383e13e3c5f4'
            'b35204fbdc0b3f4446b89fc1ac2cf84a8a68971995d0bf2e925ec7cd960f9cb3'
            'dec551ab6e7578819132c713a93c022a05d60159dc86e7a7050223577484c55a'
            '7a0aeaff4ff1a90589618835a598e545176939b97874f7abc7851caa0618f203'
            '6fac387a98bb7c37292057cffc56d62ecb629900026402633ae9160df93a8766'
            '9330f8b2ff13f34540b44e946ef35111825727b38d33286ef986142615121801'
            '613afe47fcd5fac7ccf1db93babcb082c5994d996f20b8b159f2ad1658eb5724'
            '145052bdd345b87320e369255277e3fb5152762ad123a901ef5c262dd38fe8d2'
            '0b023947811758c97c59bf9d1c188fd619ad4718dcaa767947df1cadb14f39f4'
            'c6e6ff9dcd79cff5cd969a17a545d79e84ab086e444102a591e288a8aa3ce394'
            'fa42cf4d2b7a41bc8f663a7cab4031ebafa1bf3875705bfaf8466dc60ab52c00'
            '430b4dc2b5e3861848de79627b2bedc9f3342c7da5173a14eaa5d0f8dc18ae5d'
            '2a0b5487afeab2deb2ff4e03a807ad1a03ac532ff5a2cee5d86884440c7f7671'
            'a1d728cc89cf3aee9ff92b05e62b19ee65a02b5702cff7d5a377e32c6ae29d8d'
            'b05b61dc5112cbb17e4b6cd61790d9845d13888356391624cbe7e41efeac1e75'
            'fde0e0ec90c9dfb3b4b1a0891a7dcd0e2bffde2f7efed5fe7c9bb00e5bfb915e'
            '3fdb1325a1cece981e8a296ab8f0f9b63ae357bd0784a9faaf548cc7b480707a'
            '4ca0197aee26d1ae37445ee532fefce43251d24cc7c166799f4d46817f1d3973'
            '633458d4ef8c78b72454de2d54fd6ab2e60f9e02be22f3c6104cdc8a4e0fceb9'
            'b55271e5c8c478ad3f38ad24ef34923091e0548492a266d19b3c0b4d82574c63'
            '773648b94d0e5d620f64f280777445740e61fe701025087ec8b57f45c791888b'
            '59ed5838eebb26a2bb2e58f6d5b5316989ae9d08bab10e0e6d103e656d1b0280'
            '9481c1c90cbf2ac953f07c8d4a58aa3945c425b7185c9154d67a65e4230da511'
            'd0a5c400df2834b80a4c3327b3aad3a4c4cd4de0629063962b03235697506a28'
            'd8b9f2e4c67f833b660cdb0a3523065869fb35570177239812ed4c905aeff87b'
            'acdd7c62a3665c7f6830a51635d9ac9b23ed385797f70a83bb8bafe9c572ab2b'
            '78c8292055d1c1df0cce5d180393dc8cce0abec0a7102adb6c7b1eef6016d60a'
            'eb2a7d3066da2de787b7f032c736763eb7ae5d355f81a68bab2675a96008b0bf'
            'fc7f46116c46ff9ab3eb1597a45688b6715c6e628b5c133e288e709a29bcb4ee'
            '0d00b9596d185e565c2207a0b01f8bd1a135483d02d9b7b0a54b11da8d53412e'
            'fc34b93ccb385b40dc71c6fceac4b2ad23662c7eeb248cf10d529b7e055b6ead'
            '5729f5117e208430e437df2f4843f5e5952997175992d1414f94c57d61e270b4'
            'ececcb659e7ba858fb4f10388c250a7252eb0a27373f1a72b8748afdd248e587'
            'd751e9e49156b02b44f9c1815bcb94b984cdcc4396ecc32521c739452808b134'
            '799a97264921d8623a957f6c3b9011f3b5492f557bbb7a5a19b7fa6d06ba8dcb'
            '9ed9a281f7bc9b7576e61468ba615a66a5c8cfdff42420a70aa82701a3b1e292'
            'd4b8a88685455ed29a21542a33abd9cb6510b6b129abadabdcef0f4c55bc8f61'
            'd0881ea181b1df73ff77ffaaf9c7544ecc11e82fba9b5f27b262a3c73a332555'
            '48c757948c5ede0e46177b7add2e67155f70e33c07fea8284df6576da70b3719'
            '66b7e2430c6dff6a955451e2cfc438f09cea1965a9d6f87f7e3b90decc014099'
            '1027f7680c853e056ebcec683615fb6fbbc07dbaa13b4d5d9442b146ded4ecef'
            '67c78a4d8fdf9953a5c9d458f9efe940fd97a0cab0941c075a813ac594733827'
            '1bf3c259d255ca70051b30e2e95b5446cdb8949ac4cd22c0d7fd634d89f568e2'
            '13c863f0904021b108aa8b2f55046443e6b1ebde8fd4a15c399893aae4fa069f'
            '877a4ace8713b0bcf2a4e7eec82529c029f1d0619886d18145fea96c3ffe5c0f'
            '39cab71617ae0d63f51a36d69f866391735b51691dbda63cf6f96d042b63efeb'
            'ad9cdb4b747e485a12abb0e6566612956c7a1bafa3bdb8d682c5b6d403589e48'
            '8012d3659c048a377b46d8523639fc53c5a160c24cd28e94ff348aafda91bd12'
            'e13b66accf52311f30a0db42147dadea9850cb48cd070028831ae5f5d4b856ab'
            '8be9f3dfaaffdae2972880079a491a1a8bb7cbed0b8dd7a347f668b4150a3b93'
            'b95f7c0680e4142284cf8b22c14a476e87d61b004a3a0861872b32ef7ead40a2'
            '37909eebbb50d72f9059c3b6d82c0463f2ff062c9e95845c43a6c9c0355411be'
            'e40758ed24c9b2eeb76c35fb0aebc66c626084edd827e07e1552279814c6682d'
            '645cbb3a84e60b7531617d5ae4e57f7e27308f6445f5abf653209ea76dec8dff'
            '9844ddc3a6e533d62bba727eb6c28b5d360921d5175e9ff0f1e621a5c590a4d5'
            '0ce7134b9999ecaf8bcd65542e436736ef32ddca1b3e06094cb6ec5755203b80'
            'bfe33edd8e85a12a67454e37f8c75e730830d83e313556ab9ebf9ee7fbeb3bfb'
            '3f9eec918d3f24069decb9af1554cad7c880e2da24a9afd88aca000531ab82c1'
            '77ce24cb58228fbb8aa041425bb1050850ac19177686ea6e0f41a70416f56fdb'
            'e6d5a32815ae3f33302d95fdcb2ce17862f8c65363dcfd29360480ba1001fc9c'
            '65bc07b1a8bc7c85c5f2e110c476c7389b4554ba72af57d8445ea63a576b0876'
            '2dff15bf788c671c1934e366d07e30c1814a8ef514e1af724a602e8a2fbe1b10'
            '05f29059c0c2090612e8d742178b0580d2dc940c837851ad723096f87af6663e'
            '1e28d1d997f585e54aebc3f97d39e72338912123a67330d723fdbb564d646c9f'
            '9e5c1b78ca4aae1ac06c48a526a655760685149f0d465d21f37abfe57ce075c6'
            'f78e10609fe0e0b3f4157ffab1876319b5b0db102a2c60dc4626306dc46b44ad'
            '162ee34ebcb7c64a8abebc059ce0fee27c2262618d7b60ed8faf72fef13c3650'
            'e575fab7d1e0dcb8d0c7bcf9a63ee213816ab51902e6d244a95819acacf1d4f7'
            'f90f7dce0722e95104fcb095585910c0977252f286e354b5e3bd38902cd99988'
            '9fa08315bb612088cc391249efdc3bc77536f16c91f6cf495e6fbe85b20a4a81'
            '85649ca51fd72272d7821adaf274ad91c288277713d9c18820d8499a7ff69e9a'
            '899def5c37c4fd7b2664648c28120ecec138e4d395b459e5ca34f9cce2dd77fd'
            '0cc23270f6e1808e30a928bdc84dea0b9b4136a8bc82338574f23baf47bbd280'
            '9985c9503b412198aa4197559e9a318524ebc4519c229bfa05a535828c950b9d'
            '841d1cc9bed7f9236f321df977030373f4a4163ae1a7dbfe1a51a2c1a51d9100'
            '2304e00983f87ffb38b55b444b5e3b60a884b5d30c0fca7d82fe33449bbe55ea'
            'd231dfb89cfffdbc30e7fc41579ed6066ad03abda9e567ccafae602b97ec5024'
            '7f24254aa9a54b5c858eaee2f5bccdb46aaf0e486a595ed5fd8f86ba55232a70'
            '565dd4c730b8f8b2c0fb36df6be12e5470ae10895ddcc4e9dcfbfb495de202b0'
            '33e57f83510bb73707521ebaffa789ec8caf86f9657cad665b092b581d40e9fb'
            'f31827a206f56af32e590ba56d5d2d085f558508192593743f16b2306495269f'
            'b9e0384b61958566e926dc50660321d12159025e767c18e043daf26b70104c39'
            '0ad4bb2b565bca0645f4d68c5c9af97fba094e9791da685bf83cb5f3ce74acf2'
            '79cf5c93f93228cf8efb3ba362535fb11199ac548a09ce117c9b1adc3030d706'
            '348ab060d6e63ff76337f57d7646fc3bd38d7abc0ac8c98b7acedecf2262e90f'
            'f37dccff2791ab604f9babef0ba14fbe0be30bd368dc541e2b08d07c8aa908f3'
            'e05c02b5e89bff3b946cedeca278abc628fe811e604f027c45a8aa3cf793d0eb'
            '6778b0196eefee7df739db78758e5cf9b37412268bfa5650bfeed028aed20d9c'
            '2285ddfe3054097ef4b2fe909ef8c3bcd1ea52a8f0d274416caebeef39f04a65'
            '617ee6cf8e3f66f3b4ea67a4058564628cde41901316e19f559e14c7c72c5e7b'
            'eae7b9aee968036d54dce06cebaefd919e4472e753296daccd6d344e3e2df0c2'
            '06432fb54d3be7964ecd3649233cddf80db2832f47fec34c01f65b3d9d774983'
            'a6cb138bb79a146c1bd460005623e142ef0181e3d0219cb493e02f7d08a35695'
            '413ee7dfc52ee1a4949ceeb7dbc8a33f2d6c088194d9f922fb8318faf1f01186'
            '2b192c782037fadd9cfa75548310488aabdbf3d2da73885b31bd0abd03351285'
            '92ecc6618181def0457392ccd0ee51198e065e016d1d527a7ac1b6dc7c1f09d2'
            'a87d9b8105c23642f50cbbae03d1f75d8422c5cb98ce7ee9271f7ff7505be6b8'
            'b787bebb543f8969132630c51fd0afab173a86c6abae56ff3b9e5e3e3f9f6e58'
            '464a3709c7f55f1f721e5389aa6ea4e3bc6aba669353300af094b29ffbdde1d8'
            '8fe90c1150662e858c7d5f945089b7517b0a80d8bf7ba4b1b5ffc984e7230a5b'
            'bf36173d4167ed999940f804952e6b08197cae5ad5d572eb4db150ce8ad5d58f'
            'bbd2bcb4c963f2ddae06a2efc7e9f3591312473c50c6685e1f298068316e66fe'
            '37c93d8daa9d8a012fd8ab92f088405fb202ea0b6ab73ee2482ae66af4f42091'
            '2f643b99b21452dd9bb6cb5a517cfa8342e90c6e9427a2b6274d5331dfb9ad98'
            '78b324152da65df7bb95acfcaab55e3097ceaab02fb19b228a9eb74d55f135e0'
            '3c8469b4a23b962c1396b9b451dda50ef5b283e8dd309d69033475fa9b334324'
            '5f4de44e98ddbf09375cbf4d17714d18f39195f4f4894e8524501726fd9a8a4a'
            'df1d3c3b53da64cf5760482273a98e575c651a67eec7f77df96b5b642de8f039'
            '11d3d7f243d5c5a8b9bb5d6dd2b1602c0cb0b9db1621bafc7ed66e35ff9fe092'
            '224399e74b87b5f3557511d98dff8b14089b3dadafcab6bb93eab67d3aace965'
            '5e5032e24019045c762d3c0f28f5b6b8bbf38563a65908389bf7978758920897'
            '96051b46fc183dc9cd4a223960ef37b9af631b55191852a8274bfef064cda20f'
            'c0aeb26bf5e836cc1c341c8106051b573f1766dfa05aa87f0b98be5e51b02303'
            'd640282b302c0bb0a2a8e0233ead9035e3bed871f0b7e81fe4a1ec829765db44'
            'f52b00d39961fc5b2736ea853c9cc86238e165017a493d1d5c8eac6bdc4cc273'
            'a64a92489e2744ce060c349162be1c5f33c6969234104dbd99ddb5feb08b8c15'
            '5de893c32cde5f383baa4c04c5d6dbdd735cfd4a794b0debdb2bb1b421da5ff4'
            '488016bfae457b036d996092f6cb448677611ce4449e970ceaf42695203f218a'
            '68354c5c6bd36d73ff3feceb05efa59b6acb7626617f4962be322a825e61f79a'
            '1fa76a2c86f704bdb222d66965fb3d63269ce38518b83cb0575fca855ebb6316'
            'a69bcab0ad47271a0234d9422b131806bf3968021e5dc9328caf2d4cd58557fc'
            '598beaf3cc6fdd9a5dfb1630c2800c7acd31df7aaf0f565796fba2b53ca1af1b'
            '71e2746dc3a24dd78b3cfcb7be93368c6de9963d30f43a6a73998a9cf4b17b46'
            '74523f3a35e05aba87a1d978330aef40f67b0304ac79c1c00b294c9830543db6'
            'd273983c5a657a70a3e8f2a01329822f3b8c8172b73826411a55751e404a0a4a'
            '51d515d32fb182ee37cda2ccdcb92950d6a3c2893aa280e540671c2cd0f3b1d9'
            'ed3955f1a9c7c0c15e092f9c887db08b1fc683305fdf6eb6684f22555355e202'
            '071dfc062690e90b734c0b2273ce72ad0ffa95f0c74596bc250dcfd960262841'
            '5c7398b9c8b70908f6371f47ed36737907c87c52af34c268fed0bf0ceb92ead9'
            '42f5e15c9953c5e4ccceeb2e7382a716482c34515315f7b03532b8b4e8393d2d'
            '384b8ab6d37215f3c5301a95a4accb5d64aa607f1fcb26a11b5303878451b4fe'
            '7bb71e1b3fa6ca1c61f383464aaf2bb0e2f8e772a1f01d486832464de363b951'
            '9aa2b01e1d916879f73a53d01d1d6cee68adbb31d6d9177a8cfce093cced1d50'
            '36b29691432cc9eff8b282278473b63df73bea49bc3ec5e67f31a3ae9c3ec190'
            '9cd6706dfe50d53e0f6aa09e12c034c44faacd23e966ae5a209e8bdb8f179f98'
            '6978128c8b51d8f4080631ceb2302ab51e32cc6e8615f735ee2f83fd269ae3f1'
            'f38d5652c16fde515bb1ecef450ab0f6a219d619a7274976324d5e377f7dceba'
            '93857453250e3077bd71ff98b6a65ea6621a19bb0f559a85248955ac12c45a1a'
            '2621685985a2ebf1c516881c026032ac7deafcda1a2c9b7850dc81e3dfcb64c1'
            'cbcfd20a6d4eeba40179f05735784ad32bdaef05ce8e8af05f180d45bb3e7e22'
            '51f72981ade67b1ca6adc26ec221be9f463f2b5839c7508998daa17c23d94d7f'
            'dee9efd8cdb50d719a80088b76f81aec7c41ed6d522ee750178f83883d271625'
            'bf1d70880e76bdc13ba52eafa6239ce793d85c8e43896507e43dd8984ff05b82'
            '1fd6780a80ae0c52cc120a26a1a42c1ae51b247a253e4e06113d23d2c2edd078'
            'aef8048c789fa5e851558d709946d6d79a8ff88c0440c587967f8e94bfb1216a'
            '3c80231409c20246a13fddb31776fb942c38553c51e871f8cbd687a4cfb5843d'
            'f84ac04429c13a7ff43785d75ad27569f2951ce0ffd30a3321230db2fc727216'
            '67eabc2ef2a60eb7faa00097bd1ffdb5bd28e62bf39990626a582201b7a754e5'
            '3b3cff922bd51709b605d9ead9aa71031d81447142d828eb4a6eba76fe619f9b'
            '8b870d8c151b6f2fb93e84a13146138f05d02ed11c7e7c54f8826aaaf7c9f184'
            '7edddbd0b52d732b21ad9a5fab5c704c14cd949e5e9a1ec5929a24fded1b904c'
            'f89776e4d69bb58bc6993e99ffa1d11f228b839984854c7daeb5d37f87cbe950'
            'd8a2f0d8d040d7848a709caf78912debcc3f33ee4b3cac47d73d1e1069e83507'
            '439ee305def115ba05938db6eb1644ff94165c5ab5e9420d1c1bcedbba909391'
            '85eae3c4ed2f50dcfe72643da4befc30deadb458a9b590d720cde2f2b1e97da9'
            '219cb19e96be00ab2e37d6e299658a0cfa83e52429179969b0f0121b4ac46983'
            '96de42df36bb9bba5542fe9f1a054b8cc87e172759a1868aa05c1f3acc89dfc5'
            '11ec05c52be0a07b08061f7dd003e7d7092e0472bc731b4af7bb1ef876109802'
            '9695f8df41bb4f3d222c95a67532365f569318332d03d5f3f67f37b20e6ebdf0'
            '25485360a54d6861439d60facef26de713b1e126bf015ec8f98239467a2b82f7'
            'e6401bf7b6af22f78b563665d15a22e9aef27775b79b149a66ca022468a4e405'
            '1885c039570dc00dcb4ff087a89e185fd56bae234ddc7f056a945bf36467248d'
            '69cdb34c158ceb288df11e18b4bd39de994f6657d83847bdffdbd7f346754b0f'
            'dc33ff2d4973d518d823d61aa239014831e521c75da58e3df4840d3f47749d09'
            '34af8d1a0e25924bc5b7c43c079c942339d8f0a8b57c39049bef581b46327404'
            '6db2770f06117d490610c7488547d543617b21bfa07796d7a12f6f1bd53850d1'
            'd3022b5f1df60f26e1ffddd6c66e8aa15de382ae63b3a0c1bfc0e4d3e3f325cb'
            'ec0be4795e2f6a28069bec0b5ff3e2ac9bafc99e6a9a7dc3547996c5c816922c'
            '99d9a13982dcf210057a8a78572b2217b667c3beacbf3a0d8b454f6f82837d38'
            'd1ce67fb8ba4446454d1c8dbaeda0557ff5e94d39d5e5ed7f10a65eb4c8266bc'
            '5ef8dea09a92caaf73bff7adb70b76162e5937524058a7e5bff37869cbbec293'
            '577c9b9f652b4c121fb25c6a391dd06406d3b092ba68827e6d2f09550edc54b3'
            'a7f1342a13e83e4bb9d0b793d0ea762be633f9582048c892ae9041ef39c936f4'
            '0f76fe0bd0ed4295f0321b1676732e2454024c15a35d01904ddb315afd3d545c'
            'd7dbfa023cd4e604c2553483820c5fe8aa9d71a42eea5aa77c6e7f35756612db'
            'ed2bf2547551a7053d6fdfafda3f938979645c44812fbfcda098faae3f1a362d'
            'f354300ae66f76f1c85c5f84693f0ce81d747e2c3f21a45fef496d89c960bf7d'
            'b7186006dcb21920990093f30e3dea63b7d6e977bf1256be20c3563a5db070da'
            '843bc0191f75f3e22651ae5f1e72939ab2f72a4bc30fa80a066bd66edefc24d4'
            '5276caf25ac86c8d810222b3dbb938e512c55c6831a10f3e6ed1c93b84041f1c'
            '7a2d987857b319362043e95f5353c0535c1f58eec5336fdfcf626430af7def58'
            '357703d41365b4b27c590e3ed91eabb1b663f07c4c084095e60cbed4362dff0d'
            'cfcb3a22ef46e85b45de6ee7e79d063319ebb6594faafcf1c225ea92ab6e9b92'
            '146c9e247ccc180c1f61615433868c99f3de3ae256a30a43b49f67c2d9171f34'
            'b39cdef0fa800fc44525c84ccb54a029961a8215f9619753635a9c0d2538d46d'
            'a50f4cf475b65d88e057964e0e9bb1f0aa9bbb2036dc65c64596b42932536984'
            '54e910108742c57a770f492731f99be216a52fadd361b06c8fb59d74ccc267d2'
            '4908ad288c5035a8eb12cfdf0d49270def0a268ee162b75eeee0f85d155a7c45'
            '94143f37725109f92c262ed2cf5e59bce7498c01bcc1502d7b9afe439a4e9f49'
            'd767eb0aabc880b29956c35734170f26ed551a859dbd361d140cdbeca61ab1e2'
            '9a8e94ea7f378bd32cbbd37198a4a91436180c5bb472411e48b5ec2e2124ae9e'
            '41d385c7d4ca58e59fc732af25c3983b67ac852c1a25000afe1175de458b67ad'
            'd540f220d3187173da220f885ab66608367b6574e925011a9353e4badda91d79'
            '18d26a20a969b9e3fdf2fc2d9f21eda6c40e2de84c9408bb5d3b05d499aae711'
            '83fc039473c5595ace860d8c4fafa220ff474b3fc6bfdb4293327f1a37e94d86'
            '175ee3e80ae9982737ca543e96133087cbd9a485eecc3bc4de9c1a37b47ea59c'
            '6a8b1a1a2ebf674015cc02edccce75287f1a0130d394307b36743c2f5d504b47'
            '21f60a586160667241d7702c420fc223939fb3c0bb8d3fac84f78768e8970dee'
            'a7507d819769d01a365ab707794a4084392c824f54a7a6a7862f8c3d0892b283'
            '0fda2ff0d084019ba4d7c6f371c95d8fd75ce3524c3cb8fb653a3023f6323e64'
            'd881a16cf4426aa584979d30bd82cb33429027e42122b169753d6ef1085ed6e2'
            'b75a19a7a740b25bc7944bdee6172368f988763b744e3d4dfe753f6b4ece40cc'
            'c4db69cba1110affc0e9f7bcd48bbf87b3f4fc7c61fc9155afd4c469eb3d6c1b'
            'e320a6c5ad31d271ad523dcf3ad13e2767ad8b1cb8f047f75a8aeaf8da139da2'
            '291bee647ce7310b0ea721bfd7e0525517b4468eb7c7e15eb8bd774343179702'
            '56199f7ddabf13fe5074ce809e7d3f42b42ae711800501b5b16ea82ad029c39d'
            '7a2ae44ef20feb57a68b23d846850f861394c2e02dc425a50098ae8c90267589'
            '67b1b7a3b5fe4f1376887184045fcf45c69e92af734b7aaddc05fb777b6fbd03'
            '17129e116933cf371d018bb80ae557e889637989d8638274fb25622827b03881'
            '20296a054f6fb573c1f73e49b0e3afd1efcc643548928fc9c21144f5ecf4f7e3'
            'a2eb9349b6444b326872e140eb1cf5e7c522154d69e7a0ffb0fb81c06b37543f'
            '7da8b5736845d9f2fcb837ea5d9e2628564b3b043a70948a3f0b778838c5fb4f'
            'af23d6f6c1a224baef9d3f61e287d2761385a5b88fdab4eb4c6f11aeb54c4bcf'
            '7695ce3845ea4b33927c055a39dc438a45b059f7c1b3d91d38d10355fb8cbca7'
            '72b64191b275b66ffe2469e8af2c1cfe3bafa67b529ead792a6d0160888b4237'
            '390cc9a294ab71bdb1aa2e99d13be9c753cd2d7bd6560c77118597410c4d2e87'
            'e39a2ee1fbcd360805a771e1b300f78cc88fec7b8d3e2f71cd37bbf23e725c7d'
            '0ea5d1b13ca6cff1f9231ffd62f15eefd72543dab5e468735f1a456728a02846'
            '55937e1799185b12863d447f42597ed69d9928686b8d88a1df17376a097d8369'
            '655da9c7eb6305c55742045d5a8d2037996d61d8de95806335c7c86ce0f82e9c'
            'd4ea810f0692f9f51b382fff5893887bb4580f5fa246fde546e0b13e7fcee662'
            '411c5bf740737c7918b8b1fe232dca4dc9f8e754b8ad5e20966814001ed0ac6b'
            '4676b37242ccbd1aabf56edb093a4827dc49086c0ffd764a5705899e0f35f8f7'
            '0f8daae29995a24f65619e19d8d31dea5b389f3d853d8bf297bbf607cd0014cc'
            'b6aaf5339b578ea85b50e080feb250a3e8ae8cfcdff9a461c9ec2904bc923f52'
            'f63587ca0f12b72a0600bcba1d40081f830876000bb46dd2337a3051618f4fc8'
            '4fee6c4efc90059e10f81e6d42c60a18f76588c3d74cb83a0b242a2b6c7504c1'
            '3ff15c8ecd7de3849db632e14d18d2571fa09dfc5ed93479bc4485c7a517c913'
            '91e7d9e3bb61134e77bde20dd4825b97c010155709965fedf0f49bb138e52a9d'
            '40868e7c1d2f0b8d73e4a8c7f0ff63af4f6d19be117e90bd73eb1d62cf831c6b'
            '30cfb0125f12d9c277f35663a0a33f8c30190f4e4574868a330595412d34ebf3'
            '72a2903cd7736441aac9df9d7688bd0ce48edccaadf181c3b90be801e81d3d86'
            'af407857209536a95c8e56f8231ef2c2e2aff839b22e07a1ffcbc617e9db9fa5'
            '92e1cfed4a3038bc5a127e35a2d360f145e1f4b971b551a2ba5fd7aedf7e1347'
            '84c8b9f757e028cee9fa244aea147aab2a9ec09d5325a9b01e0a49730c2b5269'
            'a3198b4b0a8e11f09dd03e133c0280504d0801269e9afa46362ffde1cbeebf44'
            '63e71662fa4b2a2c3a26f570f037eb95bb1f85397f3cd8076caed2f026a6d100'
            '7490cfa5ec963746568740651ac6781f701c9c5ea257c58e057f3ba8cf69e8da'
            'db97caf9d906fbde555dd62fa95ddba9eecfd14cb388e4f491a66d74cd5fb79a'
            '562d481066bde0658276a35467c4af00bdc6ee726305698a55b86e61d7ad82bb'
            '2896d95c02a80c6d6a5d6e953d479f5ddf2dfdb6a244441010e373ac0fb88971'
            'af4e37e9ea4401fc841ff54b9ddfc9be1079b1e89434c1a6a865dd68980f7e9f'
            '89daebc3e6fd160ac4aa9fc8b3bf71e1f74fbf92367ae71fb83a037e8bf164b9'
            'fad50862a51d693d984b07f6f6263a571554d415f5d1e16f4cbad2c266448e1b'
            'aec7103e0167479f230f6baae49dbf714074d91b2ff141983524d7417c0b98a8'
            '4064ed685c487dbc25bd3f0e9548f2e34bab9d18cefc700f9ec2dba74ba1138e'
            '9312f7c4f6ff9069b165498234ce8be658059c6728633667c526e27dc2cf1df5'
            'f6ccf251212114b54433ec949fd6a7841275f9ada20dddd2f29e9ceea4501493'
            '8fbf03860ff438702f3910ca5f28f8dac63c1c11e7efb5012b8b175493606330'
            'b4ac048d71ede7ee76d585517add45da530660ef4390e49b098733c6e897f254'
            '673aac59facbab8a9007c7f6108d11f63b603f7cabff99fabf650fea5c32b861'
            '06abde3611657adf66d383f00b093d7faecc7fa57071cce2578660c9f1010821'
            'e2e054861b4bd027cd373e18e8d8d8e6548085000e41290d95ce0c373a654b4a'
            '03c2856837ef78f57382f06b2b8563a2f512f7185d732608fd9176cb3b8edf0e'
            '0b928f33d975fc6ad9f86c8f283853ad26bdd5b10b7f1542aa2fa15e2289105a'
            '2594550bdc59b65041dba388fa2bd9690aa4675c9c332043185638a4495c685a'
            '231fdcd7ef3037e8330d8e17e61011a2c244126acc0a982f4040ac3f9f0bc077'
            '6d9b2acfb050df409c972a37d3b8e08cdea3bddb0c09db9d53137e504cfabed0'
            'ccf3ec651a847eb01de73ccad15eb7d99f80485de043efb2f370cd654f4ea44b'
            '0562428422c63773dad2c345a1882263bbf4d65cf3f42e90921f787ef5ad58e7'
            '0d759f433fa64a2d763d1340820e46e111a7a5ab75f993d1852d70b03dbb80fd'
            '48cb0d2638f8baedbc542ed444afc0644a29166f1595371af4fecf8ce1e7eeb3'
            'cefb59d5cd5f92d9dcf80e4683949f15ca4b511f4ac0a6e14d4e1ac60c6ecd40'
            'cbc538057e648b67f72a982e708d485b2efa771e1ac05fec311f9f63e5800db4'
            '0c0a6e355560527dd2d1cf7890652f4f09bb3433b6aadade4c9b5ed76de5f3ec'
            '692daff6d93d94e29e4114544ef6d5c942a7ed998b37abdc19b17136ea428eb7'
            '7de81ef35c9010270d63772bebef2f2d6d1f2d20a983d27505ac850b8c4b4296'
            '5f2ab60e120fd6eaa68d9567f3226e876684639d22a4219b313ff69ec0ccd5ac'
            '46c0cf2d539c645b448eaffec9ec494b8b19bd5077d9e58cb1ae7efece8d575b'
            '7012add459f951456ec9d6c7e6fc340b1ce15d6fc9629f8c42853412c029e57e'
            '5c839a674fcd7a98952e593242ea400abe93992746761e38641405d28b00f419'
            'ac3b87c63620426dd9b991e5ce0329eff545bccbbb34f3be09ff6fb6ab51b7b6'
            '712e227841d057c1ee1cd2fb22fa7e5a5461ae8e48fa2ca79ec42cfc1931183f'
            'b8e83a14d34d0623b51dce9581199302a221863196a1dde71a7663a4c2be9deb'
            '053e2e040ab57b9dc951b72c264860db7eb3b0200ba345b4e4c3b14f67855ddf'
            '3f316c4a2570ba26bbec722032c4099d8c8bc095efccdc15688708623367e358'
            'f0805222e57f7521d6a62e36fa9163bc891acd422f971defe97d64e70d0a4fe5'
            '7781fa89eaf60850ac3d2da7af8e5242a5ea78d1a11c49bf2910bb5a73853eb5'
            '7837d08f69c77cf6b07689544538e017c1bfcf57e34b4c0ff58e6c2cd3b37091'
            '677d2418bec65e3338edb076e806bc1ec15693c5d0104683f2efe857f61056a9'
            '282be5f36a8ce781fad8c8ae18fa3f9beff57ec1b52cb3de0789201425d9a33d'
            '1e38bc4d79ed67fd075bcc251a1c39b32a1776bbe92e5bef1f0bf1f8c531853b'
            'f2f500e4d28234f72040990ec9d39e3a6b950f9f22d3dba18416c35882612bcb'
            'ae137229bcbd6cdf0f7b80a31df61766145077ddf49416a728b02cb3921ff3fc'
            '9a2fa6e2155d7247be68c096456083145c183cbbbc2764150dda45a87197940c'
            '9b724f72796e036ab90c1021d4780d4d3d648aca59e491e6b98e725b84e99973'
            '4945f9f551b88e0d65f3db0bc25c33b8acea4d9e41163edf90dcd0b19f9069f3'
            '2b38e32f0abccf9987a4e3079dfb67dcd799fb61361e53e2882c3cbaf0d905d8'
            '32a4622180e7a0ec044bb555404c800bc9fd9ec262ec147edd5989ccd0c02cd3'
            'a9d8416fa8b42f5c947f8482c43e7d89e73a173cead56d044f6a56104a6d1b53'
            'dc35310971f3b2dbbf3f0690a219f40e2d9afcf64f9ab7cc1be722937c26b4bc'
            '09ec2a7bb152e2252b53fa7803150007879548bc709c039df7627cabbd05d469'
            'b9d782e804c2f632e395708e99a94275910eb9100b2114651e04744e9b125006'
            'a75915e7def60c94dcef72200b9a8e58e5091744960da64ec734a6c6e9b3743e'
            '8e9b5ad5ab802e97eb8e295ac6720e509ee4c243f69d781394014ebfe8bbfa0b'
            '960e6da069d81e09becb0ca57a65220ddff016ff2d6af6a223cf372a506593a3'
            '0eee52d38c090b3caa76c563b86c3a4bd71ef1a819287c19d586d7334ae8ed66'
            'fa7359d10048f68ab8b09fa71c3daccfb0e9b559aed648a8f95469c27057180c'
            '8f55c233f70c4b27f66c523580f78f1004e8b5a8b659e05a4eb49d4166cca406'
            '240948bc05c5e7c6dabba28bf89d89ffce3e303022809e73deaefe4f6ec56c66'
            '1e7ac75179f18232fe9c285163565a57ef8d3c89254a30685b57d83a38d326c2'
            '53d40abd2583d23e4718fddf1ebec84dbff8381c07cae67ff7768bbf19c6718e'
            '147a5c80aabfbf0c7d901cb5895d1de30ef2907eb21fbbab29ca94c5b08b1a78'
            '9c3842cdd74a865a8066ab39c8a7a473c0778a3f29370b5fd6b4b9aa7df4a499'
            '0b7b52767868a23d5bab768e390dc5f5c55825b6d30b86c844ff2dc7414044cc'
            '24d5b23dc417412679681396f2b49f3de8c1473deb516bd34410872eff51ed0d'
            '0ffa179e2d07eee8ad8f57493436566c7cc30ac536a3379fdf008f47f6bb7ae1'
            'ed94fce61571a4006852b7389a063ab983c02eb1bb37b47f8272ce92d06d9538'
            '589f6da84c646204747d1270a2a5661ea66ed1cced2631d546fdfb155959f9ec'
            'd6bbff5f0aada427a1e5a6da5f1f98158182f26556f345ac9e04d36d0ebed650'
            '5a5364e9d77fcdeeaa6062ced926ee3381faa2ee02d3eb83a5c27a8825540829'
            'f17a85883d4e6d00e8a97c586de764dabcc06133f7f1d55dce5cdc070ad7fe59'
            '05f360fc0b24296329c78fda852a1e9ae82de9cf7b27dae4b7f62f118f77b9ed'
            '2fb433233f2df9344722454bc7e96465c9d03bff9d77c248f9e7523fe79585b5'
            'b622b18155f7a93d1cd2dc8c01d2d6a44e08fb9ebb7b3f9e6ed101488bad6c91'
            '1cdb94821ca8a87ca9c298b5d1cbd80e2a8b67115d99f6e4551ac49e42b6a314'
            '7be68e64bf6ce8db94f63e72f0c7eb9a60d733f7e0499e628dfab0f84d6bcb97'
            'fd74ec98b9250adb3ca554bdde269adf631549f51d8a8f8f0a10b50f1cb298c3'
            'd8a8d209fdf45cf5138cbb5a506f6b52522a25afccc534d1475dad8e31105c6a'
            'e9747e91771f56fd7893e1164abd78febd14a670ceec257caad15e051de35f06'
            '2be61892e4f2b1772727be11630a62664a1826b62efa43a6fe7449521cb8744c'
            'da58575a1b2b20766513b1ec59d8e2e68db2745379f961f86650655e862d2006'
            'c6949d142f89f6916deca2232cf26a8afacf2b9fdc35ce766105e104478be599')

prepare() {
  cd "$srcdir"

  # Merge - the best ever git repo has CamelCase in it so can't use $pkgname
  holo-vendor-rust-sources -o vendored -L "InputPlumber-$pkgver/Cargo.lock" *.tar.gz

  cd "InputPlumber-$pkgver"

  mkdir -pv .cargo

  cat << EOF >> .cargo/config.toml

    [source."crates-io"]
    replace-with = "vendored-sources"

    [source."git+https://github.com/ShadowBlip/virtual-usb-rs?rev=5c4c551a23b56f627a36d6775a5876c174be9eb3"]
    git = "https://github.com/ShadowBlip/virtual-usb-rs"
    rev = "5c4c551a23b56f627a36d6775a5876c174be9eb3"
    replace-with = "vendored-sources"

    [source."git+https://github.com/emberian/evdev?rev=42b58ee08508b7799322a13bf89121a1d29cf0a2"]
    git = "https://github.com/emberian/evdev"
    rev = "42b58ee08508b7799322a13bf89121a1d29cf0a2"
    replace-with = "vendored-sources"

    [source.vendored-sources]
    directory = "${srcdir}/vendored"
EOF
}

build() {
  # Merge - the best ever git repo has CamelCase in it so can't use $pkgname
  cd "$srcdir/InputPlumber-$pkgver"

  if [[ $CARCH != x86_64 ]]; then
    export CARGO_PROFILE_RELEASE_LTO=off
  fi

  cargo build --offline --features "${_features:-}" --release --target-dir target

  # Currently this just wraps cargo build without the --offline hook we desire.
  #make
}

package() {
  cd "$srcdir/InputPlumber-$pkgver"
  make install PREFIX=${pkgdir}/usr NO_RELOAD=true
}
