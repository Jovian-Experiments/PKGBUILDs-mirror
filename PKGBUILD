...
pkgbase=mesa-nvk
# SteamOS:
# only build NVK, the rest comes from upstream/Arch Mesa
pkgname=(
#  mesa
#  opencl-mesa
#  vulkan-asahi
...
# or if you are not sure, regenerate the nvk-build-id below. To update, see
...
build() {
  local meson_options=(
    ...
    -D sysprof=false
    -D tools=drm-shim
    # SteamOS: TODO: Need https://gitlab.freedesktop.org/mesa/mesa/-/merge_requests/42728
    #-D nvk-build-id=6dc25ea2b40438949c158970db654d9db358d5b1
  )
...
#  mv vknvidia/* "$pkgdir"
  ...
  # SteamOS:
  # Install libvulkan_nouveau.so, nouveau_icd.x86_64.json and
  # 00-nvk-defaults.conf but remove the drirc file which is already installed
  # by mesa.
