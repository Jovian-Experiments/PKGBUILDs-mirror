...
options=('!strip' '!debug')
_srcname="archlinux-${pkgbase%-kasan}"
source=(
  ...
  config-kasan    # Jupiter: additional kernel fragments for enabling KASAN (overrides both prior configs)
)
sha256sums=('SKIP'
            ...
            '64c21c279b0174ca710486ff455f1c9c5c62c0a13b7ac719cd429d4549612746')
...
prepare() {
  ...
  echo "Setting config..."
  cp ../config .config
  scripts/kconfig/merge_config.sh -m ../config ../config-neptune ../config-kasan # Jupiter: merge the extra fragment
  make olddefconfig
